...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
         ...
         input_fname_pattern='*.jpg', checkpoint_dir=None, sample_dir=None, critic=5):
    """
    ...
    self.d_ln1 = batch_norm(name='d_ln1')
    self.d_ln2 = batch_norm(name='d_ln2')

    if not self.y_dim:
      self.d_ln3 = batch_norm(name='d_ln3')
  ...
  def train(self, config):
    ...
    self.g_sum = merge_summary([self.z_sum,
      self.d_loss_fake_sum, self.d__sum, self.g_loss_sum])
    self.d_sum = merge_summary(
        [self.z_sum, self.d_loss_real_sum, self.d_sum, self.d_loss_sum])
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      image = tf.reshape(image, [self.batch_size, self.input_height, self.input_width, 3])
      if not self.y_dim:
        ...
        h3 = lrelu(conv2d(h2, self.df_dim*8, name='d_h3_conv', sn=True))
        h3 = attention_layer(h3, name="d_att")
  ...
  def generator(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      if not self.y_dim:
        ...
        h0 = linear(z, s_h16*s_w16*self.gf_dim*8, 'g_h0', sn=True)
        h0 = tf.reshape(h0, [self.batch_size, s_h16, s_w16, self.gf_dim*8])
        h0 = tf.nn.relu(self.g_bn0(h0))

        self.h1= deconv2d(h0, [self.batch_size, s_h8, s_w8, self.gf_dim*4], name='g_h1', sn=True)
        h1 = tf.nn.relu(self.g_bn1(self.h1))

        h2 = deconv2d(h1, [self.batch_size, s_h4, s_w4, self.gf_dim*2], name='g_h2', sn=True)
        h2 = tf.nn.relu(self.g_bn2(h2))

        h3 = deconv2d(h2, [self.batch_size, s_h2, s_w2, self.gf_dim*1], name='g_h3', sn=True)
        h3 = tf.nn.relu(self.g_bn3(h3))

        h3 = attention_layer(h3, name="g_att")

        h4 = deconv2d(h3, [self.batch_size, s_h, s_w, self.c_dim], name='g_h4')
        h4 = tf.nn.tanh(h4)

        return tf.reshape(h4, [self.batch_size, -1])
      else:
        s_h, s_w = self.output_height, self.output_width
  ...
  def sampler(self, z, y=None):
    with tf.variable_scope("generator") as scope:
      ...
      if not self.y_dim:
        ...
        h1 = deconv2d(h0, [self.batch_size, s_h8, s_w8, self.gf_dim*4], name='g_h1', sn=True)
